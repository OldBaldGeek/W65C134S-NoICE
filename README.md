...
The WDC tools do support some symbol options, including the mysterious MicroTek format.
Failing discovery of clay tablets documenting that format, we wrote wdcsym_noi.py to
extract symbol definitions and line-number information from text files generated by
WDC's WDCSYM.
...
- Dump a text version of the debug file using **WDCSYM** with flags **-a** to get an
Aux table, **-l** to get line numbers, and **-s** to get symbols. Redirect the 
console output to a text file
- Run **wdcsym_noi.py** specifying the text file
...
around this, wdcsym_noi.py allows an optional **-S** or **--start** switch to let you define
